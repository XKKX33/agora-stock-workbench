...
export interface DeepJudgment { ts_code: string; rank: number; score: number; analysts: { methodology: Analyst; sentiment: Analyst; technical: Analyst } }
...
export interface JudgmentResult {
  ...
  deep: DeepJudgment[]; final: FinalJudgment[]; usage?: { input_tokens?: number; output_tokens?: number };
}
...
export function validateJudgmentRequest(input: unknown): JudgmentRequest {
  ...
  if (limits.coarse > 20 || limits.deep > 20 || limits.final > 3) throw new Error("limits exceed Top20→Top3 contract");
...
export function validateJudgmentResult(input: unknown, request: JudgmentRequest, runId: string): JudgmentResult {
  ...
  const deepCodes = new Set<string>(); const deep = x.deep.map((item: unknown, i: number) => { const y = object(item); const code = text(y.ts_code, "deep.ts_code"); if (!coarseCodes.has(code) || deepCodes.has(code) || y.rank !== i + 1) throw new Error("deep subset/rank"); deepCodes.add(code); const analysts = object(y.analysts); return { ts_code: code, rank: y.rank, score: finiteScore(y.score, "deep.score"), analysts: { methodology: validateAnalyst(analysts.methodology, "methodology"), sentiment: validateAnalyst(analysts.sentiment, "sentiment"), technical: validateAnalyst(analysts.technical, "technical") } }; });
  ...
  return { protocol_version: x.protocol_version, workflow_version: x.workflow_version, run_id: runId, trade_date: x.trade_date, candidate_hash: x.candidate_hash, input_hash: x.input_hash, coarse, deep, final, usage: x.usage };
}
