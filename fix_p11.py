...
old_match = re.search(r'<style>.*?</style>', html, re.DOTALL)
if not old_match:
    print('No <style> found')
    sys.exit(1)

old_style = old_match.group()
...
new_style = """<style>
    ...
    .agent-status { font-size: 12px; color: var(--text-muted); max-width: 340px; text-align: right; line-height: 1.7; }
